...
<script>
  ...
  //接口方法
  import {getBVO_basic,getBVO_stores,editBVO_basic,addBVO_stores} from "../api/BVO_main_api.js"
  ...
  export default {
    name: "BVOMain",
    components: {HollowDotsSpinner, VaModal, VaCard, VaPagination, VaButton, MyCube, VaInput},
    created : function() {
      /*
        调用getBVO_basic与getBVO_stores 接口方法

      getBVO_basic(this,this.$store.state.userId).then(
        res => {
          this.bvo_name = res.data.name;
          this.edit_bvo_name = this.bvo_name;
          this.dsrId = res.data.dsrId;
    ...
    methods : {
      ...
      confirmAddNewStore : function(){
        if(this.newStoreName == "" || this.newStorePlatform == 0){
          ...
          this.isloading = true;
          let _this = this;
          setTimeout(() => _this.isloading = false,3000);
         /*
          addBVO_stores(this,{
            dsrId : this.dsrId ,
            platformType : this.newStorePlatform,
